...
while (my $line = <LEX>) {
	...
	#$Lex{$token}{$lemma} = $tag;
        $Lex{$token}++;
        $StopWords{$token} = $tag if ($tag =~ /^(P|I|R|D|U)/);
        $i++;
    } 

    if  ($#entry == 2 && $entry[2] eq "NNP") { ##identificar formas nao ambiguas que sao nomes proprios
	    $Noamb{$token}=1;
...
sub ner_en {
	...
	elsif ( ($tokens[$i] =~ /^$UpperCase/) && !$Lex{$tokens[$i]} && 
